...
# -*- coding: utf-8 -*-
"""
Визуализация кодов, полученных методом main_da, сначала сжатых
классическим MDS, а затем отображённых в 2D с помощью UMAP.

Берёт изображения указанных цифр из набора MNIST, кодирует их по правилам
approach main_da и строит 2D-раскладку: сначала классический MDS по
косинусному расстоянию с выходом в промежуточное пространство, затем UMAP
для финальной визуализации. Точки раскрашиваются по цифрам.
...
MDS_COMPONENTS = 50  # размерность после MDS перед UMAP
...
def pairwise_cosine(bits: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    dist = pairwise_cosine(bits)
    mds_coords = classical_mds(dist, n_components=MDS_COMPONENTS)
    reducer = umap.UMAP(n_components=2, random_state=SEED)
    coords = reducer.fit_transform(mds_coords)

    fig, ax = plt.subplots(figsize=(6, 6))
    for digit, color in DIGIT_COLORS.items():
        mask = labels == digit
        ax.scatter(coords[mask, 0], coords[mask, 1], c=color,
                   s=POINT_SIZE, label=str(digit), marker=',',
                   antialiased=True, linewidths=0, alpha=0.5)
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_aspect("equal", "datalim")
    ax.legend(title="digit")
    ax.set_title("Korvin approach MDS+UMAP")
    fig.tight_layout()
